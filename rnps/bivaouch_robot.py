...
class BivaOuchServerSession(BaseServerSession):
    def __init__(self, factory, address):
        BaseServerSession.__init__(self, factory, address)
        self.set_protocol("BIVAOUCH")
        return
    ...
    def dataReceived(self, data):
        ...
        while len(self.receive_buffer) > 0:
            ...
            if soup_type == 'S' or soup_type == 'U':
                ...
                msg._payload = bivaouch.get_message(soup_type, msg.message)
...
class BivaOuchServerFactory(BaseServerFactory):
    def __init__(self, robot, name, port, version):
        BaseServerFactory.__init__(self, robot, name, port, version)
        self.set_protocol("BIVAOUCH", BivaOuchServerSession)
        return


class BivaOuchClient(BaseClient):
    def __init__(self, factory, address):
        self.set_protocol("BIVAOUCH")
...
class BivaOuchClientFactory(BaseClientFactory):
    def __init__(self, robot, name,
        ...
        BaseClientFactory.__init__(self, robot, name,
                                   server_host, server_port,
                                   protocol_version, client_port)
        self.set_protocol("BIVAOUCH", BivaOuchClient)
        return
...
class BivaOuchRobot(OuchRobot):
    def __init__(self):
        self.protocol_name = "BIVA OUCH"
        self.client_factory = BivaOuchClientFactory
        self.server_factory = BivaOuchServerFactory
        self.framing_protocol = soupbin
        self.message_protocol = bivaouch
